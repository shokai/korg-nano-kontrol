(function() {
  ...
  module.exports = nanoKONTROL2 = (function(superClass) {
    ...
    function nanoKONTROL2(input, name1) {
      this.input = input;
      this.name = name1;
      nanoKONTROL2.__super__.constructor.call(this, {
        wildcard: true,
        delimiter: ':'
      });
      this.input.on('message', (function(_this) {
        return function(deltaTime, message) {
          var buttons, code, i, index, name, ref, ref1, ref2, ref3;
          debug(message);
          for (index = i = 0; i <= 7; index = ++i) {
            if (message[1] === index) {
              _this.emit("slider:" + index, message[2]);
              return;
            }
          }
          ...
          for (index in ref) {
            code = ref[index];
            if (message[1] === code) {
              _this.emit("knob:" + index, message[2]);
              return;
            }
          }
          ...
          for (index in ref1) {
            code = ref1[index];
            if (message[1] === code) {
              _this.emit("button:s:" + index, message[2] > 0);
              return;
            }
          }
          ...
          for (index in ref2) {
            code = ref2[index];
            if (message[1] === code) {
              _this.emit("button:m:" + index, message[2] > 0);
              return;
            }
          }
          ...
          for (index in ref3) {
            code = ref3[index];
            if (message[1] === code) {
              _this.emit("button:r:" + index, message[2] > 0);
              return;
            }
          }
          buttons = {
          ...
          for (code in buttons) {
            name = buttons[code];
            if (message[1] === code - 0) {
              _this.emit("button:" + name, message[2] > 0);
              return;
            }
          }
        };
